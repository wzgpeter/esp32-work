...
BaseType_t xReturn = pdFALSE;
	...
	{
		( void ) memcpy( ( void * ) pxQueue->pcWriteTo, (void *)pvItemToQueue, ( size_t ) pxQueue->uxItemSize ); /*lint !e961 !e418 MISRA exception as the casts are only redundant for some ports, plus previous logic ensures a null pointer can only be passed to memcpy() if the copy size is 0. */
	...
	{
		( void ) memcpy( ( void * ) pxQueue->u.pcReadFrom, (void *)pvItemToQueue, ( size_t ) pxQueue->uxItemSize ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
